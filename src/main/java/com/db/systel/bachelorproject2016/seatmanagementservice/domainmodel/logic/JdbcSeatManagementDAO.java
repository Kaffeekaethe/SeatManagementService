...
public class JdbcSeatManagementDAO implements SeatManagementDAO {
	...
	public Integer insert(String type, DatabaseObject databaseObject) {
		...
		try {
			...
			/*
			 * Baue die Query dynamisch zusammen, in Abhängigkeit davon, wie
			 * viele Properties es gibt
			 */
			...
			/*
			 * Das Datenbankobjekt teilt uns ja nun mit, auf welche Properties
			 * wir zugreifen können. D.h. wir nehmen das Objekt und suchen uns
			 * die Properties mit den festgelegten Namen (z.B. routeID). Dadurch
			 * müssen wir nicht explizit getRouteID() etc. aufrufen, sondern
			 * können generisch vorgehen
			 */
			for (int i = 0; i < databaseObject.properties.length; i++) {

				try {
					/*
					 * Die Werte der Properties werden dann in das
					 * PreparedStatement gesetzt. Wichtig dafür ist, dass die
					 * Reihenfolge der Properties im Array gleich den Attributen
					 * in der SQL-Anfrage sind
					 */
	...
	public DatabaseObject update(Integer id, String type, DatabaseObject databaseObject) {
		...
		try {
			...
			/*
			 * Auch hier wieder die Properties anhand ihres Namens generisch
			 * ermittelt und in das Statement schreiben. Genauer beschrieben bei
			 * Insert
			 */
			for (int i = 0; i < databaseObject.properties.length; i++) {
	...
	public List<Integer> disable(Integer id, String type) {
		...
		try {
			/*
			 * Wenn ein Platz oder Wagen disabled wird, dann wollen wir ein
			 * Update durchführen (State = ...) und wir wollen alle Plätze
			 * ermitteln, die davon betroffen sind. Deswegen 2 Anfragen
			 */
			String disable = IOUtils.toString(classLoader.getResourceAsStream("Queries/disable/" + type), "UTF-8");

			String selectSeatIDs = IOUtils.toString(classLoader.getResourceAsStream("Queries/select/" + type), "UTF-8");
	...
	public DatabaseObject select(Integer id, String type) {
		...
		try {
			...
			/*
			 * Je nachdem, welcher Typ angegeben ist, wird ein konkretes
			...
			if (RS.next()) {
				...
				for (int i = 0; i < databaseObject.properties.length; i++) {
					...
					try {
						/*
						 * Die Werte der Properties werden dann in das Objekt
						 * gesetzt
						 */
						Class<?> c = databaseObject.getClass();
						Field f = c.getDeclaredField(databaseObject.properties[i]);
						 ...
						 * 
						 * Ist grundsätzlich nicht nötig, vermeidet aber ggf.
						 * Errors
						 */
