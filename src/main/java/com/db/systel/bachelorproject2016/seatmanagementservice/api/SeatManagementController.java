...
import com.db.systel.bachelorproject2016.seatmanagementservice.clients.NotificationServiceClient;
...
public class SeatManagementController {
	...
	/*
	 * Einige Werte sollen durch den Manager in menschlich lesbarer Form
	 * eingegeben werden, aber in der Datenbank als Long auftauchen.
	 * 
		...
		try {
			...
			case "train_connection":
				...
				if (json.has("departureTime")) {
					...
					json.put("departureTime", dateTimeFormat.parse("01-01-1970 " + departureTime + ":00").getTime());
			...
			case "seat_allocation":
				...
				if (json.has("departureTime")) {
					...
					String departureTime = json.getString("departureTime");
					json.remove("departureTime");
					json.put("departureTime", dateTimeFormat.parse(departureTime + ":00").getTime());
				...
				if (json.has("arrivalTime")) {
					...
					json.put("arrivalTime", dateTimeFormat.parse(arrivalTime + ":00").getTime());
				}
			...
			case "route_station":
				/*
				 * Kommt als Minutenzahl rein, also einfach 60 s / min * 1000 ms
				 * / s
				 */
			...
			case "train_connection_cancellation":
				...
				if (json.has("day")) {
					...
					json.remove("day");
					json.put("day", dateTimeFormat.parse(day + " 00:00:00").getTime());
				}
	...
	/*
	 * Alle Objekte leiten von einem Datenbankobjekt ab. Das ermöglicht es, sich
	 * erst später auf einen konkreten Datentyp festzulegen und die Statements
	 * schön zu kapseln
	 */
	private static DatabaseObject parseObject(String type, String jsonString) {
		...
		/*
		 * Der Mapper funktioniert von selbst (also benötigt keine
		 * Implementierung durch uns). An dieser Stelle ggf. einfach einen neuen
		 * Case hinzufügen, wenn noch ein Datenobjekt dazu kommt
		 */
		try {
			switch (type) {
			case "route":
	...
	public ResponseEntity<?> insert(@RequestParam(value = "type") String type, @RequestBody String input) {
		...
		try {
			...
			/*
			 * Tag in lesbarer Form für Weiterleitung an NotificationService
			 * speichern
			 */
			...
			/*
			 * Für manche Eingaben müssen die Werte noch in das Long-Format für
			 * die Datenbank umgewandelt werden
			 */
			...
			/*
			 * Wenn das möglich war, dann dieses Objekt einfügen. Die anderen
			 * Funktionen machen das analog. Die Datenbank entscheidet dann, wo
			 * sie das Objekt einfügt
			 */
			if (obj != null) {
				...
				if (insertionID != null) {
					/*
					 * Wenn eine Zugverbindung gecancelled wurde, dann müssen
					 * Kunden benachrichtigt werden
					 */

					if (type.equals("train_connection_cancellation")) {
						NotificationServiceClient.notifyTrainConnectionCancelled(
	...
	public ResponseEntity<?> update(@RequestParam(value = "id") int id, @RequestParam(value = "type") String type) {
		...
		try {
			...
			/*
			 * Beim disable kommt eine Liste mit allen betroffenen Plätzen
			 * zurück. Diese muss an den Benachrichtigungsdienst weiter geleitet
			 * werden, damit Kunden informiert werden
			 */

			System.out.println("Received request to disable. Type: " + type + ", Id: " + id);
			List<Integer> seatIDs = SeatManagementService.seatManagementDAO.disable(id, type);

			NotificationServiceClient.notifySeatsDisabled(seatIDs);
	...
	public ResponseEntity<?> seatAllocationUpdate(@RequestParam(value = "type") String type,
			@RequestParam(value = "trainConnectionID", required = false) Integer trainConnectionID,
			@RequestParam(value = "seatID", required = false) Integer seatID,
			@RequestParam(value = "arrivalTime", required = false) String arrivalTime,
			@RequestParam(value = "departureTime", required = false) String departureTime, @RequestBody String input)
		...
		/*
		 * Wenn das möglich war, dann dieses Objekt einfügen. Die anderen
		 * Funktionen machen das analog. Die Datenbank entscheidet dann, wo sie
		 * das Objekt einfügt
		 */
		if (seatAllocation != null) {
			...
			if (type.equals("delete")) {
				...
				if (obj != null) {
					...
					SeatAllocation newObject = SeatManagementService.seatManagementDAO
							.updateSeatAllocation(seatAllocation, (SeatAllocation) obj);
					if (newObject != null) {
